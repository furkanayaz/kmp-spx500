    ...
    install(StatusPages) {
        exception<Throwable> { call, throwable ->
            when (throwable) {
                ...
                is SerializationException -> call.respond(HttpStatusCode.BadRequest, Response.Error<Nothing>(code = 400, errorMessages = listOf("Occurred a serialization error.")))
                is BadRequestException -> call.respond(HttpStatusCode.BadRequest, Response.Error<Nothing>(code = 400, errorMessages = listOf(throwable.message ?: "Your entered field(s) cannot null or empty.")))
                is ConstraintViolationException -> {
                    ...
                    call.respond(HttpStatusCode.BadRequest, Response.Error<Nothing>(errorMessages = listOf(errorMessage)))
                }
                else -> call.respond(HttpStatusCode.InternalServerError, Response.Error<Nothing>(code = 500, errorMessages = listOf("unknown.error")))
            }
        }
    }
}